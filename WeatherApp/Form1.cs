...
namespace WeatherApp
{
    public partial class WeatherForm : Form
    {
        ...
        private void comboBox1_SelectionChangeCommitted(object sender, System.EventArgs e)
        {
            ...
            List<Weather> _MeasureUnits = _weather.Weathers.Where(x => x.MeasureUnit.ToString() == q).ToList();
        ...
        {
            List<Weather> _Plus = _weather.Weathers.Where(x => x.Temperature > 0).ToList();
            WeatherDataGrid.DataSource = _Plus;
        }

        private void button3_Click(object sender, System.EventArgs e)
        {
            List<Weather> _Sort = _weather.Weathers.OrderBy(x => x.Temperature).ToList();
            WeatherDataGrid.DataSource = _Sort;
        }
    }
}